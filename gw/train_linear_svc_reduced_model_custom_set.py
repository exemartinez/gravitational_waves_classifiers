...
gw_roc_file = './models_images/gw_linear_svc_reduced_custom_roc.png'
...
#Custom testing sets...
images_validation = images[0:41,:]
labels_validation = labels[0:41]
images = images[41:82,:]
labels = labels[41:82]

model  = svm.SVC(kernel='linear', C =0.0005, probability=True, class_weight="balanced", verbose=0, random_state=random_state)
...
avg_precision = average_precision_score(labels_validation, preds)

print("AUC score: %s" % roc_score)
print("AVG Precision score: %s" % avg_precision)

fpr, tpr, threshold = metrics.roc_curve(labels_validation, preds)

print ("Thresold: %s" % threshold)
