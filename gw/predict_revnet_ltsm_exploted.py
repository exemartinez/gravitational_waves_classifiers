...
gw_img_file='./dataset/gw_images/gw_%s_%s_revnet_exploted_lstm_.png'
gw_roc_file = './models_images/gw_revnet_exploted_lstm_%s_roc.png'

SHAPE_SIZE_X = 140
SHAPE_SIZE_Y = 170
max_features = SHAPE_SIZE_X * SHAPE_SIZE_Y
OPTIMAL_UMBRAL = 0.75555
...
def plot_metrics(model,images,labels,dataset):
    ...
    pred_labels = (prediction > OPTIMAL_UMBRAL) * 1 #because we got a binary classification: 1 - gw, 0 - no gw
...
def identify_predicted_gw(subdataset):
    ...
    prediction = model.predict(images, verbose=True, batch_size=151)

    class_prediction = (prediction > OPTIMAL_UMBRAL) * 1

    #sum(class_prediction)
    print("Predict REVNET --- %s seconds ---" % (time.time() - start_time))

    print(" Recursive Neural Network")
    print( "Amount of Gravitational Waves identified by the model: %s" % str(prediction[class_prediction > 0 ].shape[0]))
    ...
    images_gw = images[np.where(class_prediction > 0)[0]]
    indexes = np.where(class_prediction > 0)[0]
